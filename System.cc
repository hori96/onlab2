...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // cout << "mTrackedMapPoints" << endl;
    // std::vector<cv::KeyPoint> kp_object = mTrackedKeyPointsUn;
    // std::vector<ORB_SLAM3::MapPoint*> dadada = mTrackedMapPoints;

    // std::fstream outputFile;
    // outputFile.open( "outputFile.pcd", std::ios::out );
    // cout << kp_object.size( );
    // for( size_t ii = 0; ii < kp_object.size( ); ++ii ){
    //     cout << kp_object[ii].pt <<std::endl;
    //     //outputFile << kp_object[ii].pt <<std::endl;
    //     outputFile << kp_object[ii].pt.x << " " << kp_object[ii].pt.y <<std::endl;
    // }
    // outputFile.close( );

    vector<MapPoint*> vMPs = mpAtlas->GetAllMapPoints();
    if (!vMPs.empty()){
        MapPoint* pMP = vMPs.back();
        if(pMP->isBad())
            return Tcw;
    ...
    if (!vpKFs.empty()){
        ...
        // ifstream f3;
        // f3.open("keyframe_real_time.txt");

        // string lastLine = "";

        // if(f3.is_open()) {
        //     f3.seekg(-1,ios_base::end);                // go to one spot before the EOF

        //     bool keepLooping = true;
        //     while(keepLooping) {
        //         char ch;
        //         f3.get(ch);                            // Get current byte's data

        //         if((int)f3.tellg() <= 1) {             // If the data was at or before the 0th byte
        //             f3.seekg(0);                       // The first line is the last line
        ...
        //             f3.seekg(-2,ios_base::cur);        // Move to the front of that data, then to the front of the data before it
        //         }
        //     }
            
        //     getline(f3,lastLine);                      // Read the current line
        //     cout << "Result: " << lastLine << '\n';     // Display it
        //     f3.close();
        // }

        std::stringstream current_odom;
        current_odom << setprecision(6) << pKF->mTimeStamp << setprecision(7) << " " << t.at<float>(0) << " " << t.at<float>(1) << " " << t.at<float>(2)
            << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
        
        std::string current_odom_s = current_odom.str();
        // cout << current_odom_s;

        ofstream f4;
        f4.open("keyframe_real_time.txt", std::ios_base::app);
        f4 << current_odom_s;
        f4.close();

        // if(lastLine!=current_odom_s){
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    cout << endl << "Saving map points to " << "point_cloud.pcd" << " ..." << endl;
    vector<MapPoint*> vMPs = mpAtlas->GetAllMapPoints();
    ...
    cv::Mat MPPositions = pMP->GetWorldPos();
    f2 << setprecision(7)<< MPPositions.at<float>(0) << " " << MPPositions.at<float>(1) << " " << MPPositions.at<float>(2) << endl;
    }
    f2.close();
